...
Shrinking::
A parameter `shrink: ('a -> 'a Iter.t)` can be provided along with a random
generator. If so, when a counter-example is found for a property, the
counter-example will be shrunk recursively as long as it continues refuting
the property; this allows to find smaller counter-examples. However, shrinking
can be slow. A parameter `~max_fail:int` can be given to the test
